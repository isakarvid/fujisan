...
if not os.path.ismount(inspool):
  if not os.path.isdir(inspool):
    os.makedirs(inspool)
  os.system("mount_smbfs //FRONTIER:FRONTIER@" + sambahost + "/InSpool " + inspool);
...
if not os.path.ismount(outspool):
  ...
  os.system("mount_smbfs //FRONTIER:FRONTIER@" + sambahost + "/OutSpool " + outspool);
...
def exportimages(exportimages, ordername, extension = ".tif", path = outdir):
  ...
  for image in exportimages:
    ...
    log("trying Image.open(" + image.imgfile + ")")
    try:
